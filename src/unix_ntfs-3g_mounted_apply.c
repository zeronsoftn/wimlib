...
struct unix_ntfs_3g_mounted_apply_ctx {
...
unix_compute_path_max(const struct list_head *dentry_list,
		      const struct unix_ntfs_3g_mounted_apply_ctx *ctx)
{
	size_t max = 0;
	size_t len;
...
unix_build_extraction_path(const struct wim_dentry *dentry,
			   struct unix_ntfs_3g_mounted_apply_ctx *ctx)
{
	char *pathbuf;
	char *p;
	const struct wim_dentry *d;
...
static void
unix_reuse_pathbuf(struct unix_ntfs_3g_mounted_apply_ctx *ctx)
{
	ctx->which_pathbuf = (ctx->which_pathbuf - 1) % NUM_PATHBUFS;
}
...
apply_linux_xattrs(int fd, const struct wim_inode *inode,
		   const char *path, struct unix_ntfs_3g_mounted_apply_ctx *ctx,
...
unix_create_hardlinks(const struct wim_inode *inode,
		      ...
		      const char *first_path, struct unix_ntfs_3g_mounted_apply_ctx *ctx)
{
	const struct wim_dentry *dentry;
	const char *newpath;
...
static void
unix_cleanup_open_fds(struct unix_ntfs_3g_mounted_apply_ctx *ctx, unsigned offset)
{
 ...
 * buffer.  */
static bool
prepare_data_buffer(struct unix_ntfs_3g_mounted_apply_ctx *ctx, u64 blob_size)
{
...
{
	struct unix_ntfs_3g_mounted_apply_ctx *ctx = _ctx;
...
static int
unix_set_dir_metadata(struct list_head *dentry_list, struct unix_ntfs_3g_mounted_apply_ctx *ctx)
{
	const struct wim_dentry *dentry;
	int ret;
...
{
	int ret;
	struct unix_ntfs_3g_mounted_apply_ctx *ctx = (struct unix_ntfs_3g_mounted_apply_ctx *)_ctx;
	size_t path_max;
	u64 dir_count;
...
const struct apply_operations unix_ntfs_3g_mounted_apply_ops = {
	...
	.context_size           = sizeof(struct unix_ntfs_3g_mounted_apply_ctx),
};
